    ...
    try {
      ...
      if (user === validUser && pass === validPass) {
        ...
        return new Response(null, {
          status: 302,
          headers: { 'Location': '/dashboard-brickj.html', 'Set-Cookie': cookie }
        });
      }
...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    body{background:#060608;color:#f0f0f0;font-family:'Inter',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;background-image:radial-gradient(ellipse at 30% 30%,rgba(232,255,0,0.05) 0%,transparent 60%)}
    .box{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:16px;padding:40px 36px;width:100%;max-width:380px;backdrop-filter:blur(20px);box-shadow:0 40px 80px rgba(0,0,0,0.5)}
    ...
    input{width:100%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#f0f0f0;font-family:'Inter',sans-serif;font-size:0.9rem;padding:11px 14px;outline:none;border-radius:8px;transition:border-color 0.2s}
    input:focus{border-color:rgba(232,255,0,0.5);background:rgba(232,255,0,0.03)}
    button{width:100%;background:#e8ff00;color:#000;border:none;font-family:'Bebas Neue',sans-serif;font-size:1.1rem;letter-spacing:0.1em;padding:13px;border-radius:8px;cursor:pointer;margin-top:24px;transition:opacity 0.2s}
...
<body>
  <div class="box">
    ...
    <form method="POST" action="/dashboard-brickj.html">
